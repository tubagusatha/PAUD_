...
@section('content')
    <div class="p-20 bg-slate-100">
        <div class="container mx-auto p-10 shadow-xl border bg-white rounded-xl ">
            ...
            <div class="bg-gray-100 flex flex-col p-4 rounded-lg mb-6">
                ...
                <span class="text-sm text-teal-700">
                    Untuk meneruskan permohonan, silahkan periksa terlebih dahulu data pengajuan permohonan PAUD. Anda harus
                    melihat berkas-berkas lampiran terlebih dahulu dengan menekan tombol Lihat File agar dapat
                    menindaklanjuti permohonan ini.
                </span>
            </div>
            ...
            <div class="bg-gray-100 p-6 rounded-lg shadow-lg space-y-4 mt-4">
                ...
                <form action="{{ route('front_office.update', $permohonan->id) }}" method="POST">
                    @csrf
                    @method('PATCH')

                    <!-- Radio button untuk status -->
                    <div class="flex items-center mb-4">
                        <input type="radio" id="diteruskan" name="status_id" value="2" class="mr-2" required>
                        <label for="diteruskan" class="text-gray-700">Teruskan Ke kadis</label>
                    </div>
                    ...
                    @php
                        $isDisabled =
                            !$permohonanGallery ||
                            !$permohonanGallery->surat_permohonan_perpanjang ||
                            !$permohonanGallery->surat_pernyataan_keabsahan ||
                            !$permohonanGallery->surat_izin_pendirian ||
                            !$permohonanGallery->peninjauan_lokasi;

                        $emptyFiles = [];
                        if (!$permohonanGallery || !$permohonanGallery->surat_permohonan_perpanjang) {
                            $emptyFiles[] = 'Surat Permohonan Perpanjang';
                        }
                        if (!$permohonanGallery || !$permohonanGallery->surat_pernyataan_keabsahan) {
                            $emptyFiles[] = 'Surat Pernyataan Keabsahan';
                        }
                        if (!$permohonanGallery || !$permohonanGallery->surat_izin_pendirian) {
                            $emptyFiles[] = 'Surat Izin Pendirian';
                        }
                        if (!$permohonanGallery || !$permohonanGallery->peninjauan_lokasi) {
                            $emptyFiles[] = 'Peninjauan Lokasi';
                        }
                    @endphp

                    <button class="bg-green-500 text-white rounded py-1 w-full my-10" {{ $isDisabled ? 'disabled' : '' }}>
                        Disposiskan
                    </button>

                    @if ($isDisabled)
                        <div class="text-red-500 mt-4">
                            <p>Tidak bisa melanjutkan proses, File surat Tidak lengkap</p>
        ...
        <div class="text-center mt-10">
            @if ($role->role == 'FO')
                <h1 class="text-m">Masuk Sebagai Front Office</h1>
