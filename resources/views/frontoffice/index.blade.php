...
@include('layout.nav.sidebarfo.sidebar') 

@if (session('login_message'))
    <div id="loginPopup" class="fixed z-50 inset-0 flex items-center justify-center bg-black bg-opacity-50 transition-opacity duration-300 ease-out">
        <div class="bg-white p-6 rounded-lg shadow-lg transform transition-transform duration-500 ease-out scale-95 opacity-0" id="popupContent">
            ...
            <div class="text-center opacity-0 transition-opacity delay-700" id="popupButton">
                <button onclick="closePopup()" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-10 rounded focus:outline-none focus:shadow-outline">Tutup</button>
            </div>
        </div>
    </div>
    ...
    <style>
        ...
        @keyframes checkmarkBounce {
            ...
            60% {
                transform: scale(0.9);
                color: #16a34a; /* hijau lebih gelap */
            }
            100% {
                transform: scale(1);
                color: #10b981; /* hijau stabil */
            }
        }

        /* Terapkan animasi pada icon checklist */
        .animate-checkmark {
            animation: checkmarkBounce 1s cubic-bezier(0.68, -0.55, 0.27, 1.55) forwards; /* Lebih smooth dan elastis */
        }
            ...
            <div class="text-center w-full">
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
                    @foreach($statuses as $status)
                        ...
                        <div class="bg-{{ $randomColor }} text-white p-6 rounded-lg shadow-md flex flex-col items-center justify-center h-32">
