                            ...
                            <tbody>
                                @foreach ($permohonans as $permohonan)
                                    <tr
                                        ...
                                        <td class="px-6 py-4">
                                            @if ($permohonan->status->id == 11)
                                                <a href="{{ route('kadis.show.finish', $permohonan->id) }}"
                                                    class="bg-blue-500 py-1 px-5 rounded-sm font-medium text-white dark:text-blue-500 hover:underline">
                                                    Detail
                                                </a>
                                            @else
                                                @if ($permohonan->status->id < 3)
                                                    <a href="{{ route('kadis.show', $permohonan->id) }}"
                                                        class="bg-blue-500 py-1 px-5 rounded-sm font-medium text-white dark:text-blue-500 hover:underline">
                                                        Detail
                                                    </a>
                                                @else
                                                    @if ($permohonan->status->id == 13)
